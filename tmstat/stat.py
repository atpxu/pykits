...
def get_raw_data(fname: str = 'time_stat.xlsx'):
    ...
    return df_filtered[df_filtered['start'] >= morning_6_am]
...
def write_single_day_summary(df: pd.DataFrame, fname: str = 'day_stat.xlsx'):
    ...
    grouped = df.groupby(['name', 'date']).agg(
        earliest_start_time=('start', 'min'),
        latest_end_time=('end', 'max')
    ).reset_index()
    # 计算duration，单位为秒
    grouped['work_start'] = grouped.apply(
        lambda row: datetime.combine(row['date'], row['earliest_start_time']), axis=1)
    grouped['work_end'] = grouped.apply(
        lambda row: datetime.combine(row['date'], row['latest_end_time']), axis=1)
...
def write_total_summary(grouped: pd.DataFrame, fname: str = 'total_stat.xlsx'):
    ...
    result['lack_ratio'] = (result['lack_days'] / result['total_days']) * 100  # lack天数占比%
    result['ok_ratio'] = ((result['full_days'] + result['over_days']) / result['total_days']) * 100  # full+over天数占比%
    # 将统计结果写入新的 Excel 文件
